...
analysis.par$out.dir.DATA <- 'c9orf72/driver_Frontal_Cortex/DATA'
...
analysis.par$out.dir.PLOT <- "c9orf72/driver_Frontal_Cortex/PLOT/Visualization/" # Users can modify this path
...
sig_driver <- draw.volcanoPlot(dat=ms_tab,label_col='gene_label',logFC_col=sprintf('logFC.%s_DA',comp_name),
                               Pv_col=sprintf('P.Value.%s_DA',comp_name),logFC_thre=0.1,Pv_thre=0.05,
...
sig_gene <- draw.volcanoPlot(dat=ms_tab,label_col='geneSymbol',logFC_col=sprintf('logFC.%s_DE',comp_name),
                             Pv_col=sprintf('P.Value.%s_DE',comp_name),logFC_thre=1,Pv_thre=0.05,
...
draw.heatmap(mat=exp_mat,use_genes=ms_tab[driver_list,'originalID'],use_gene_label=ms_tab[driver_list,'geneSymbol'],
             ...
             row_names_gp = gpar(fontsize = 12), pdf_file=sprintf('%s/heatmap_Frontal_Cortex_Exp.pdf',analysis.par$out.dir.PLOT))
...
draw.heatmap(mat=ac_mat,use_genes=driver_list,use_gene_label=ms_tab[driver_list,'gene_label'],
             ...
             cluster_rows=TRUE,cluster_columns=TRUE,show_column_names=FALSE, clustering_distance_rows='pearson',clustering_distance_columns='pearson',
             row_names_gp = gpar(fontsize = 12),pdf_file=sprintf('%s/heatmap_Frontal_Cortex_AC.pdf',analysis.par$out.dir.PLOT))
...
draw.categoryValue(ac_val=ac_mat[use_driver,],exp_val=exp_mat[ms_tab[use_driver,'originalID'],],use_obs_class=use_obs_class,
                   ...
                   pdf_file=sprintf('%s/categoryValue_COL4A2_AC&EXP.pdf',analysis.par$out.dir.PLOT))
# 只生成activity表达水平差异
draw.categoryValue(ac_val=ac_mat[use_driver,],exp_val=NULL,use_obs_class=use_obs_class,class_order=c('ALS','Healthy'),
                   pdf_file=sprintf('%s/categoryValue_COL4A2_AC.pdf',analysis.par$out.dir.PLOT))
...
draw.categoryValue(ac_val=ac_mat[use_driver,],exp_val=exp_mat[ms_tab[use_driver,'originalID'],],use_obs_class=use_obs_class,
                   ...
                   pdf_file=sprintf('%s/categoryValue_COL4A2_group&genotype.pdf',analysis.par$out.dir.PLOT))
...
# 
# ##
write.table(sig_driver,'/Users/mosquito/Desktop/sig_driver_Frontal_Cortex.csv')
write.table(sig_gene,'/Users/mosquito/Desktop/sig_gene_Frontal_Cortex.csv')
